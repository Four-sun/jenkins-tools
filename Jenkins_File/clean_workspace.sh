...
project_list="dt-center-dataScience,dt-center-tagapp,dt-center-engine,dt-assets,dt-center-api,dt-center-gateway,dt-center-ide,dt-center-streamapp,dt-center-valid,dt-public-service"
...
function while_clean() {
  ...
  gc_clean_project_path=`find $gc_project_path -maxdepth 1 -type f -size +1M`
  gc_array=(${gc_clean_project_path// / })
  for project in ${gc_array[@]}
	do
	    echo  "清理时间："$current_time ,"当前文件超过预定大小,日志文件地址："$project >> $clean_log
      echo "" > $project
  done
...
function clean_project(){
	...
	do
	    ...
	    gc_project_path=$project_base_path$PROJECT$clean_file$gc
	    echo "当前清理项目:"$PROJECT
	    echo "当前清理项目地址:"$project_path
	    echo "当前清理项目地址:"$gc_project_path
      while_clean
	done
}
