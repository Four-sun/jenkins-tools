...
-- name catalog_create_two
-- type FlinkSQL
-- author admin@dtstack.com
-- create time 2022-11-23 11:55:01
-- desc 
CREATE DATABASE IF NOT EXISTS fanshu_catalog_1.fanshu_database;
CREATE DATABASE fanshu_catalog.fanshu_database;
DROP DATABASE fanshu_catalog.fanshu_database;
CREATE TABLE IF NOT EXISTS fanshu_catalog_1.fanshu_database.fanshu_table_1(
    id int
 );
...
-- 创建结果表
CREATE TABLE if not exists fanshu_catalog.fanshu_database.fanshu_table_3(
    id      int comment'id_data',
    name    string comment'name_data',
    age     bigint comment'age_data',
    primary key(id) not enforced
 )with(
    'connector'='print'
 );
