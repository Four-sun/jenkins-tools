...
CREATE TABLE postgresqlSideTableOne(
    id bigint,
...
CREATE TABLE postgresqlSideTableTwo(
    id bigint,
    integer_data int,
    varchar_10_data varchar,
    PRIMARY KEY(id,integer_data) NOT ENFORCED
 )WITH(
    'password' = '******',
    'connector'='postgresql-x',
...
CREATE TABLE postgresqlSideTableThree(
    id bigint,
    integer_data int,
    varchar_10_data varchar,
    PRIMARY KEY(id) NOT ENFORCED
...
-- desc PostgreSQL维表-LRU、ALL、None、三种不同类型
INSERT 
INTO
    kafkaResultTable
    select
    ...
    from
        sourceTable st
    left join
        postgresqlSideTableOne for SYSTEM_TIME as of st.proc_time as mst 
            on st.id = mst.id;
