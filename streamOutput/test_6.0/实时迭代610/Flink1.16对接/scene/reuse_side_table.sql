...
CREATE TABLE d_form_filling_record(
    after ROW <form_subtitle varchar,
...
INTO
    test_table
    SELECT
        a.after.form_subtitle as jcdbt,
        a.after.check_method as jcff_bm,
        '' as jcff_mc,
        a.after.check_classification as jcfl_bm,
        c.dict_value as jcfl_mc,
        a.after.check_result as jcjg_bm,
        d.dict_value as jcjg_mc
    FROM
        d_form_filling_record a
    LEFT JOIN
        dim_d_form_dict for SYSTEM_TIME as of a.proc_time as b
            ON a.check_method = b.dict_code
    LEFT JOIN
        dim_d_form_dict for SYSTEM_TIME as of a.proc_time as c
            ON a.check_classification = c.dict_code
    LEFT JOIN
        dim_d_form_dict for SYSTEM_TIME as of a.proc_time as d
