...
WHERE  cc.component_type_code = 0
       ...
       IF @flink_componentversion is not null THEN
                update rdos_batch_task set component_version = @flink_componentversion where task_type = 2 and tenant_id = @tenantId;
            end if;
...
WHERE  cc.component_type_code = 1
       ...
       IF @spark_componentversion is not null THEN
                update rdos_batch_task set component_version = @spark_componentversion where task_type in (0, 1, 3) and tenant_id = @tenantId;
            end if;
   FETCH id_cur INTO dtUicTenantId;
END WHILE;
   CLOSE id_cur;
