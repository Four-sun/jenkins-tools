BEGIN;
UPDATE `rdos_stream_param_template` SET `param` = '## sql任务并发度设置
...
# async.side.poolSize=5',`gmt_modified` = now() WHERE `task_type` = 0 AND `component_version` IN ('1.8','1.10') AND is_deleted = 0;
COMMIT;